 ...
 *
 * Design: budget.model_rates + tier-level default rates for estimates when
 * adapters only return tokens. USD is always best-effort.
 */
 ...
 * Used only as last-resort estimate for observability / pressure.
 */
 ...
 * Returns null when tokens missing and no rate, or rate cannot be resolved
 * when tokens are present but no rate source exists.
 */
export function estimateUsdFromTokens(params: {
  ...
  return (input / 1_000_000) * rate.in_per_mtok + (output / 1_000_000) * rate.out_per_mtok;
}
...
export function resolveEstimatedUsd(params: {
  ...
  const fromRates = estimateUsdFromTokens(params);
  if (fromRates !== null) {
